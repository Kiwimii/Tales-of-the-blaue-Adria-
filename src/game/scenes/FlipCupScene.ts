...
export class FlipCupScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(480, 574, 'Tippen oder Leertaste · maximal 3 Versuche', {
      fontFamily: 'system-ui, sans-serif',
      fontSize: '15px',
      color: '#d7ccb0',
    }).setOrigin(0.5);
  ...
  private drawTimingBar(): void {
    ...
    this.add.rectangle(480, 205, 120, 28, 0xe4bd55, 0.95);
    this.add.rectangle(480, 205, 54, 28, 0x79d59d, 0.98);
  ...
  private finish(won: boolean): void {
    gameStore.advanceMinutes(15);
    gameStore.setFlag(won ? 'flipCupWon' : 'flipCupTried');
    this.message.setText(won
      ? 'Sauberer Flip. Die Runde ist gewonnen und dein Ruf im Südlager steigt.'
      : 'Drei Versuche sind vorbei. Das Südlager applaudiert trotzdem aus Höflichkeit.');
