...
export function playBattleIntro(
  ...
  const overlay = scene.add.container(0, 0).setDepth(500).setSize(960, 640)
    .setInteractive(new Phaser.Geom.Rectangle(0, 0, 960, 640), Phaser.Geom.Rectangle.Contains);
  ...
  const status = scene.add.text(480, 607, `${statusLead(snapshot)} · TIPPEN ZUM ÜBERSPRINGEN`, {
  ...
  let phraseIndex = 0;
  const phraseTimer = scene.time.addEvent({
    delay: 360,
    repeat: 3,
    callback: () => {
  ...
  overlay.on('pointerup', complete);
  const keyboard = scene.input.keyboard;
  const skipKey = keyboard?.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE);
  skipKey?.once('down', complete);
  scene.time.delayedCall(2180, () => {
    if (!overlay.active) return;
    phrase.setText('FRUSTKAMPF!').setFontSize(34);
    scene.cameras.main.shake(180, 0.006);
    scene.time.delayedCall(260, complete);
  });
  scene.time.delayedCall(2900, complete);
}
...
function statusLead(snapshot: GameSnapshot): string {
  ...
  if (snapshot.needs.hangover >= 28) return 'STATUS: KATER · Frustresistenz reduziert';
