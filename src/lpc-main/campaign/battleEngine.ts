...
export function resolveBattleTurn(current: BattleState, moveId: CombatMoveId, snapshot: GameSnapshot, teamSize: number, progression?: CombatProgressionContext, signature = false, random: () => number = Math.random): BattleTurnResult {
  ...
  activeState = state; publishBattle(state); window.dispatchEvent(new CustomEvent('lpc-campaign-attack-use', { detail: { id: moveId, success: hit } }));
...
function publishBattle(state: BattleState): void { window.dispatchEvent(new CustomEvent('lpc-campaign-battle-state', { detail: structuredClone(state) })); }
