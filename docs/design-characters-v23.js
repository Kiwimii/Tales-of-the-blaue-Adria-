...
function badge(id,x,y,t){const s=S(),rel=s.relations[id]||0,h=hour();let icon='';if(id==='gundula'&&s.world.hedgeWet)icon='💢';else if((id==='schubert'||id==='schima')&&h>=19)icon='💨';else if(rel>20)icon='♥';if(!icon)return;ctx.save();ctx.translate(x+22,y-60+Math.sin(t*.005)*3);ctx.fillStyle='rgba(255,250,225,.95)';ctx.beginPath();ctx.arc(0,0,14,0,Math.PI*2);ctx.fill();D.text(icon,0,1,14,'#222','center',800);ctx.restore()}
...
function worldCharacters(t){const s=S();const moved=Math.hypot(s.world.x-last.x,s.world.y-last.y)>1.2;last={x:s.world.x,y:s.world.y,t};
 const g=guards();const all=[['gundula',g.gundula],['uli',g.uli],...C.friendIds.map(id=>[id,friend(id)])];if(s.day<=2&&hour()>=17&&!s.flags.rivalWon)all.push(['ronny',{x:1580,y:500}]);
