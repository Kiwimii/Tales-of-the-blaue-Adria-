...
for (const game of ['flipCup', 'beerPong', 'flunkyball', 'hedgePee', 'maslHole']) {
  ...
  assert(runtimeSceneMarker(game), `${game} scene class is not generated by the runtime.`);
}
...
for (const marker of ['.graphics-v3-brawl', '.graphics-v3-patrol', '.graphics-v3-vision-cone', '.graphics-v3-table.flip', '.graphics-v3-table.pong', '.graphics-v3-flunky-lane', '.graphics-v3-hole']) assert(css.includes(marker), `CSS marker is missing: ${marker}`);
...
function runtimeSceneMarker(game) {
  return readFileSync(resolve(process.cwd(), 'src/lpc-main/campaign/graphicsUpdateV3.js'), 'utf8').includes(`graphics-v3-scene-${game}`)
    || readFileSync(resolve(process.cwd(), 'src/lpc-main/campaign/graphicsUpdateV3.js'), 'utf8').includes('graphics-v3-scene-${game}');
}
